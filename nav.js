    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 /*La requête est terminée*/ && (xhr.status == 200 || xhr.status == 0)) {
            ...
            if (is_admin) {
                ...
                document.getElementById('li_comptes').style.display = "block";
...
function nbComptesAValider(){
    xhr = new XMLHttpRequest();
    xhr.open("POST", "http://localhost/JSValAcro/countNoValidProfil.php");
    /*xhr.upload.onprogress = function(e) {
        progress.value = e.loaded;
        progress.max = e.total;
    }*/
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 /*La requête est terminée*/ && (xhr.status == 200 || xhr.status == 0)) {
            //Requête entièrement terminé
            console.log('rep :::> '+xhr.responseText);
            console.log(parseInt(xhr.responseText, 10));
            if (parseInt(xhr.responseText, 10) <= 1){
                $('#li_valid_profil_Number').html('<b>'+xhr.responseText +'</b> Compte à valider');
            }
            else {
                $('#li_valid_profil_Number').html('<b>'+xhr.responseText +'</b> Comptes à valider');
            }
